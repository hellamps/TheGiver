	...
	handle_register(request, response) {
		...
		this.redis.hgetall(redis_giver_config, (error, config) => {
			...
			_.forEach(parsed_config, (uris, mapping) => {
				if (mapping === request.body.endpoint) {
					if (!(uris.indexOf(request.body.uri) > -1)) {
						uris.push(request.body.uri);
						hsets.push({key: mapping, value:JSON.stringify(uris)});
					}
				}
				else {
					if (uris.indexOf(request.body.uri) > -1) {
						_.pull(uris, request.body.uri);
						if (uris.length > 0) {
							hsets.push({key: mapping, value:JSON.stringify(uris)});
						}
						else {
			...
			if (!parsed_config[request.body.endpoint]) {
				hsets.push({key: request.body.endpoint, value:JSON.stringify([request.body.uri])});
			}
			async.parallel([
				(callback) => {
					async.parallel(
